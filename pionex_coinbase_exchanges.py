...
from pionex_coinbase_config import PionexCoinbaseConfig

logger = logging.getLogger(__name__)

class PionexCoinbaseExchangeManager:
    """
    Exchange manager for Pionex.US and Coinbase Pro
    Handles all exchange interactions with proper error handling and rate limiting
    """
    
    def __init__(self):
        self.config = PionexCoinbaseConfig
        self.exchanges = {}
    ...
    def _initialize_exchanges(self):
        ...
        try:
            ...
            self.exchanges['coinbasepro'] = ccxt.coinbasepro({
                ...
                'enableRateLimit': True,
                'rateLimit': self.config.EXCHANGE_CONFIGS['coinbasepro']['rateLimit'],
            })
            
            if self.config.COINBASE_SANDBOX:
                self.exchanges['coinbasepro'].set_sandbox_mode(True)
                logger.info("✅ Coinbase Pro initialized in TESTNET mode")
            else:
                logger.info("✅ Coinbase Pro initialized in PRODUCTION mode")
