...
Tests all critical fixes and validates system functionality
"""
...
class ComprehensiveTestSuite:
    ...
    async def _test_error_handling(self):
        ...
        try:
            ...
            # Trigger multiple errors to test circuit breaker
            for i in range(6):  # Exceed threshold
                await self.error_handler.handle_error(
                    Exception(f"Test error {i}"), context, ErrorType.BALANCE_ERROR
                )
    ...
    async def _test_thread_safety(self):
        ...
        try:
            ...
            # Test 3: Thread-safe trade execution
            try:
                trade_result = await self.thread_safe_manager.execute_thread_safe_trade(
                    'binance', 'TON/USDT', 'buy', 10, 2.5
                )
                if not trade_result:
                    errors.append("Thread-safe trade execution failed")
            except Exception as e:
                warnings.append(f"Trade execution failed (expected in mock): {str(e)}")
            
            # Test 4: Thread safety statistics
    ...
    async def _test_integration(self):
        ...
        try:
            ...
            # Test 2: Error handling + Thread safety
            context = ErrorContext(
            ...
            try:
                await self.thread_safe_manager.execute_thread_safe_trade(
                    'binance', 'TON/USDT', 'buy', position_size, 2.5
                )
            except Exception as e:
                # This is expected in mock environment
                await self.error_handler.handle_error(e, context, ErrorType.TRADE_ERROR)
    ...
    async def _test_performance(self):
        ...
        try:
            ...
            if balance_time > 5.0:  # 5 seconds for 100 operations
                warnings.append(f"Balance validation slow: {balance_time:.2f}s for 100 operations")
            ...
            if position_time > 3.0:  # 3 seconds for 50 operations
                warnings.append(f"Position sizing slow: {position_time:.2f}s for 50 operations")
            ...
            for i in range(100):
                ...
                await self.error_handler.handle_error(
                    Exception(f"Performance test {i}"), context, ErrorType.UNKNOWN_ERROR
                )
            error_time = time.time() - error_start
            
            if error_time > 2.0:  # 2 seconds for 100 operations
                warnings.append(f"Error handling slow: {error_time:.2f}s for 100 operations")
            ...
            self.test_results.append(TestResult(
                ...
                status=status,
                execution_time=execution_time,
                details=f"Balance: {balance_time:.2f}s, Position: {position_time:.2f}s, Error: {error_time:.2f}s",
                errors=errors,
                warnings=warnings
            ))
    ...
    async def _test_stress(self):
        ...
        try:
            ...
            # Test 3: Error recovery under stress
            for i in range(50):
                context = ErrorContext(
                ...
                await self.error_handler.handle_error(
                    Exception(f"Stress test {i}"), context, ErrorType.UNKNOWN_ERROR
                )
            ...
            self.test_results.append(TestResult(
                ...
                status=status,
                execution_time=execution_time,
                details=f"Handled 20 concurrent operations and 100 high-frequency operations",
                errors=errors,
                warnings=warnings
            ))
